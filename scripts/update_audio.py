...
with open('scripts/generate-audio.ts', 'r') as f:
    content = f.read()

# 1. Change Voice
content = content.replace("'LAWYER PIG': 'Kore'", "'LAWYER PIG': 'Puck'")

# 2. Update SCRIPT
old_script = """const SCRIPT = [
...
new_script = """const SCRIPT: Array<{ character: Character, text: string, translation?: string, ttsText?: string }> = [
  { character: 'DETECTIVE PIG', text: "Is that so, Wolfy? Well, I'm not afraid of you! And I'm taking you in!" },
  { character: 'WOLF', text: "Is that so?" },
  { character: 'DETECTIVE PIG', text: "Yeah, that's right!" },
  { character: 'WOLF', text: "For what crime?" },
